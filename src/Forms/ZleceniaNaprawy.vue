...
async function set_auto_nr_faktury() {
    ...
    const prefix_search = `/__/${now_year}`;
    const nr_str = COLS.nr_faktury.get_full_sql();
    const sql_res = await ipc.db_query(`
        SELECT 
            CAST(substr(${nr_str},1,length(${nr_str})-${prefix_search.length}) AS INTEGER) as suffix 
        FROM ${TAB.get_full_sql()}
        WHERE (${COLS.nr_faktury.get_full_sql()} LIKE '%${prefix_search}')
